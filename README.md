# helloworld - Argo CD Application (PROD)

This repository contains the Argo CD Application manifest for the **PROD** environment of the helloworld essesseff™ app.  
...
```
helloworld-argocd-prod/
├── app-of-apps.yaml.template                      # Root Application template
├── argocd/
│   └── helloworld-prod-application.yaml.template # PROD environment Application manifest (auto-synced) template
...
│   └── offboard-helloworld-prod.sh                  # script for offboarding the helloworld prod app 1) from essesseff only or 2) from Argo CD and K8s entirely
├── setup-argocd-cluster.sh           # Argo CD K8s setup script 
├── setup-argocd.sh                   # Argo CD helloworld-prod essesseff app setup script 
└── README.md                         # This file
```
...
### Configure helloworld-prod essesseff App to Argo CD and deploy to K8s

1. **Git Clone This Repository**:
   ```bash
   git clone git@github.com:essesseff-helloworld-springboot-templat/helloworld-argocd-prod.git
   ```
...
2. **Configure Environment Variables in .env File**:
   ...
   Then set the environment variables in the .env which will be used for generating the following from the templates in this repository:
      ...
      - `helloworld-argocd-prod` repository (to read Application manifests)
      - `helloworld-config-prod` repository (to read Helm charts and values)
      ...
      c. ***Configuration of helloworld-prod Deployment in Argo CD***:

      helloworld-prod-application.yaml is used to configure the helloworld-prod deployment

      d. ***Configuration of helloworld-argocd-prod App-of-Apps Deployment in Argo CD***:

      app-of-apps.yaml is used to configure the helloworld-argocd-prod app-of-apps deployment

      e. ***Configuration of helloworld-prod Argo CD Notifications to essesseff***:

      If notifications-secret.yaml is downloaded from essesseff for helloworld-prod, notifications-configmap.yaml will be used to configure Argo CD notifications to essesseff.
...
4. **Run the setup-argocd.sh Script**:
   ...
   This script applies all secrets, configmaps, Argo CD application definitions, etc. for helloworld PROD.
...
5. **Verify in Argo CD UI**:
   ...
   You should see:
   - `helloworld-argocd-prod` - Root Application (watches this repository)
   - `helloworld-prod` - Environment Application (auto-synced by root Application)

6. **Access the Deployed Application**:
   ```bash
   kubectl port-forward service/helloworld-prod 8081:80 -n essesseff-helloworld-springboot-templat
   # Access: http://localhost:8081
   ```
### How to Offboard helloworld-prod Deployment from Argo CD and K8s

1. **Execute the offboarding script**:
   ```bash
   cd offboarding
   chmod 744 offboard-helloworld-prod.sh
   ./offboard-helloworld-prod.sh
   ```
...
- **Namespace**: `argocd`
- **Source Repository**: `helloworld-config-prod`
...
3. essesseff GitHub App automation triggers essesseff to auto-update Helm `helloworld-config-prod/Chart.yaml` and `helloworld-config-prod/values.yaml` with the image tag of the newly built image
4. Argo CD syncs PROD Application automatically on K8s

### Manual GitOps Deployment

1. Push update(s) to the `main` branch in `helloworld-config-prod` repository, typically to Helm Chart.yaml and/or values.yaml
...
- **Config PROD**: `https://github.com/essesseff-helloworld-springboot-templat/helloworld-config-prod`
- **Argo CD PROD**: `https://github.com/essesseff-helloworld-springboot-templat/helloworld-argocd-prod` (this repo)
...
- Allow Argo CD to detect changes in `argocd/helloworld-prod-application.yaml` more quickly
